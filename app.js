    ...
    if (tas_state == 'upload') {
        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].id == 'temp') {
                var cin = {ctname: upload_arr[i].ctname, con: sensing.get[i]()};
...
function humi_upload_action() {
    if (tas_state == 'upload') {
        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].id == 'humi') {
                var cin = {ctname: upload_arr[i].ctname, con: sensing.get[i]()};
                console.log(JSON.stringify(cin) + ' ---->');
                upload_client.write(JSON.stringify(cin) + '<EOF>');
            }
...
wdt.set_wdt(require('shortid').generate(), 4, humi_upload_action);
